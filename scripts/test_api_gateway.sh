...
aws --endpoint-url "$AWS_ENDPOINT_URL" --region "$AWS_REGION" lambda list-functions

echo ""
echo "2. Testing HTTP APIs:"
aws --endpoint-url "$AWS_ENDPOINT_URL" --region "$AWS_REGION" apigatewayv2 get-apis

echo ""
echo "3. Testing HTTP API Routes:"
# Get the first API ID
API_ID=$(aws --endpoint-url "$AWS_ENDPOINT_URL" --region "$AWS_REGION" apigatewayv2 get-apis --query 'Items[0].ApiId' --output text 2>/dev/null)
