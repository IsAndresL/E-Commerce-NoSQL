...
def lambda_handler(event, context):
    ...
    if not service.user_has_order(user_id, order_id):
        return {"statusCode": 404, "body": json.dumps({"detail": "Order not found for user"})}

    details = service.get_order_details(order_id)
    if not details:
        return {"statusCode": 404, "body": json.dumps({"detail": "Order details not found"})}
    return {"statusCode": 200, "body": json.dumps(details.model_dump())}
